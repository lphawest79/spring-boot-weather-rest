...
public class WeatherControllerAdviceTest {
	...
	public void testHandleClientErrorException() {
		ResponseEntity<WeatherErrorCode> err = 
				controllerAdvice.handleClientErrorException(new HttpClientErrorException(HttpStatus.TOO_MANY_REQUESTS), webRequest);
		...
		err = controllerAdvice.handleClientErrorException(new HttpClientErrorException(HttpStatus.BAD_GATEWAY), webRequest);
		assertEquals("Error", err.getBody().getStatus());
	...
	public void testHandleMissingParameterException() {
		...
		ResponseEntity<WeatherErrorCode> err = 
				controllerAdvice.handleMissingParameter(new MissingServletRequestParameterException("city", "String"), webRequest);
		assertEquals("Error", err.getBody().getStatus());
		assertEquals(HttpStatus.BAD_REQUEST.value(), err.getBody().getCode());
	}
	...
	public void testHandleDefault() {
		ResponseEntity<WeatherErrorCode> err = 
				controllerAdvice.handleDefault(new RuntimeException("Failed to load"), webRequest);
		assertEquals("Error", err.getBody().getStatus());
